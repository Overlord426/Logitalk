...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.label_theme = CTkOptionMenu(self.frame, values=['Темна', 'Світла', "Зелена"], command=self.change_theme)
        ...
        # --- Емодзі ---
        self.emoji_button = CTkButton(self.bottom_frame, text='😀', width=40, command=self.open_emoji_panel)
        self.emoji_button.pack(side="right", padx=(0,5))
    ...
    def change_theme(self, value):
        if value == 'Темна':
            ...
            set_appearance_mode('dark')
            # простий зелений режим
            green = "#00a000"
            hover = "#009000"
            self.save_btn.configure(fg_color=green, hover_color=hover)
            self.send_button.configure(fg_color=green, hover_color=hover)
            self.emoji_button.configure(fg_color=green, hover_color=hover)
        else:
            set_appearance_mode('light')


    def open_emoji_panel(self):
        if hasattr(self, 'emoji_win') and self.emoji_win.winfo_exists():
            self.emoji_win.lift()
            return

        self.emoji_win = Toplevel(self)
        self.emoji_win.title("Емоції")
        self.emoji_win.geometry("300x200")

        emojis = ['😀','😂','🤣','😊','😍','😎','😢','😭','😡','👍','👎','🙏','💖','🔥','🎉','🌈','🍎','🍕','⚽','🎵']

        frame = Frame(self.emoji_win)
        frame.pack(fill='both', expand=True)

        row, col = 0, 0
        for e in emojis:
            btn = Button(frame, text=e, width=3, command=lambda em=e: self.insert_emoji(em))
